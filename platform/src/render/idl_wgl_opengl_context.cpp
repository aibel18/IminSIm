...
bool idl::OpenGLContext::init(int major, int minor, u8 color, u8 depth) {
  ...
  const int pixelAttribs[] = {
      ...
      8, // TODO: create a parameter to set this field
      0,
  };
  ...
  int contextAttribs[] = {
      ...
      minor,
  // WGL_CONTEXT_PROFILE_MASK_ARB,	WGL_CONTEXT_CORE_PROFILE_BIT_ARB, // TODO: verify in another window
  // versions
  ...
  if (!dc) {
    // TODO: Add Exception Manager
    MessageBoxA(NULL, "Recover Handle of context failed for dummy", "Error!",
  ...
  if (!wglChoosePixelFormatARB(dc, pixelAttribs, 0, 1, &sharedPixelFormat, &formats) || formats == 0) {
    // TODO: Add Exception Manager
    MessageBoxA(NULL, "OpenGL dons't support required pixel format!", "Error!",
                MB_OK | MB_ICONEXCLAMATION);
    ReleaseDC(dummy, dc);
    DestroyWindow(dummy);
    return false;
...
bool idl::OpenGLContext::makeCurrent(idl_window* window) {
  ...
  if (!sharedPixelFormat || !SetPixelFormat(window->dc, sharedPixelFormat, &pfd)) {
    // TODO: Add Exception Manager
    MessageBoxA(NULL, "Setup pixel format failed for Current Window", "Error!",
                MB_OK | MB_ICONEXCLAMATION);
    return false;
  }
  ...
  if (!window->context) {
    // TODO: Add Exception Manager
    MessageBoxA(NULL, "Context Creation failed for Current Window", "Error!",
                MB_OK | MB_ICONEXCLAMATION);
    return false;
  }
